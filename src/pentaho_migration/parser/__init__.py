...
from pentaho_migration.parser.powercenter import PowerCenterParser
from pentaho_migration.parser.talend import TalendParser
...
def detect_parser(path: str | Path):
    ...
    if "<Report" in head and (".rpt" in head or "RptToXml" in head or "<ReportDefinition" in head):
        raise ParseError(
            ...
            "or `pentaho-migrate report <file>`."
        )
