...
namespace RptToXml
{
	public partial class RptDefinitionWriter : IDisposable
	{
		...
		private void GetTable(CrystalDecisions.ReportAppServer.DataDefModel.Table table, XmlWriter writer)
		{
			...
			bool redact = System.Environment.GetEnvironmentVariable("RPTTOXML_REDACT") == "1";
			WriteAttributeString(writer, "UserName", redact ? "" : table.ConnectionInfo.UserName);
			WriteAttributeString(writer, "Password", redact ? "" : table.ConnectionInfo.Password);
			writer.WriteEndElement();
		...
		private void WriteFieldFormat(FieldObject fo, CRReportDefModel.ISCRReportObject rasrdmRo, XmlWriter writer)
		{
			try
			{
				...
				try
				{
					var df = ff.DateFormat;
					if (df != null)
					{
						writer.WriteStartElement("DateFieldFormat");
			...
			string rasThousands, string rasPosition)
		{
			try
			{
				...
				string negative;
				if (negStyle.IndexOf("Bracket", StringComparison.OrdinalIgnoreCase) >= 0 ||
					negStyle.IndexOf("Parenthes", StringComparison.OrdinalIgnoreCase) >= 0)
					negative = "(" + positive + ")";
				else if (negStyle.IndexOf("Trailing", StringComparison.OrdinalIgnoreCase) >= 0)
					negative = positive + "-";
				else
					negative = "-" + positive;
			...
			string rasSep1, string rasSep2)
		{
			try
			{
				string day = "d", month = "M", year = "yyyy";
				try
				{
					string d = df.DayFormat.ToString();
					day = d.IndexOf("LeadingZero", StringComparison.OrdinalIgnoreCase) >= 0 ? "dd"
						: d.IndexOf("No", StringComparison.OrdinalIgnoreCase) == 0 ? "" : "d";
				}
				catch { }
				try
				{
					string m = df.MonthFormat.ToString();
					month = m.IndexOf("LeadingZero", StringComparison.OrdinalIgnoreCase) >= 0 ? "MM"
						: m.IndexOf("Short", StringComparison.OrdinalIgnoreCase) >= 0 ? "MMM"
						: m.IndexOf("Long", StringComparison.OrdinalIgnoreCase) >= 0 ? "MMMM"
						: m.IndexOf("No", StringComparison.OrdinalIgnoreCase) == 0 ? "" : "M";
				}
				catch { }
				try
				{
					string y = df.YearFormat.ToString();
					year = y.IndexOf("Short", StringComparison.OrdinalIgnoreCase) >= 0 ? "yy"
